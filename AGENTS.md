...
**Fix: serve mode embeds queries with each repo's indexed model.**

Problem: `CodesearchService::new_for_serve` hardcoded `ModelType::default()`
(384-dim `minilm-l6-q`) and `run_serve` never received `--model`, so every
semantic query on the hub used the default model regardless of what each index
recorded. On indexes rebuilt with `embeddinggemma-q4` (768-dim) this surfaced as
`Query embedding dimension mismatch: expected 768, got 384`. The single-repo
stdio path already resolved the model from `metadata.json` (issue #118 fix); the
multi-repo serve path never did.

Changes:
- `ModelType::from_index_metadata(db_path)` — reader counterpart to
  `write_metadata_fields`, used by every query path.
- `EmbeddingServicePool` (embed) — lazily loads one `EmbeddingService` per
  model, shared across MCP sessions and REST handlers.
- `ServeState::model_for_alias(alias)` + `embedding_pool()`.
- `CodesearchService::query_model(alias)` / `embedding_service_for(model)`
  replace the single shared `Option<EmbeddingService>`.
- `semantic_search` (project) and `semantic_search_multi` (group) resolve the
  model per repo; the fan-out helper passes the alias to the closure so each
  store is searched with its own model's query embedding.
- CLI warns that `serve --model` is inert; README documents per-repo resolution.

Tests:
- `model_for_alias_*` (serve): metadata-driven, per-repo, None without metadata.
- `serve_service_uses_repo_model_not_default` (serve): the service seam.
- `test_model_type_round_trips_through_index_metadata` (embed).
